...
class Database:
    ...
    def initialize(self):
        ...
        with self.get_connection() as conn:
            ...
            # Run migrations
            self._migrate_notification_settings(conn)

    def _migrate_notification_settings(self, conn):
        """Add new columns for position open/close notifications if they don't exist."""
        cursor = conn.cursor()
        ...
        if 'email_position_opened' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN email_position_opened BOOLEAN DEFAULT 1")

        if 'email_position_closed' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN email_position_closed BOOLEAN DEFAULT 1")

        if 'telegram_position_opened' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN telegram_position_opened BOOLEAN DEFAULT 1")

        if 'telegram_position_closed' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN telegram_position_closed BOOLEAN DEFAULT 1")

        if 'telegram_take_profit' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN telegram_take_profit BOOLEAN DEFAULT 1")

        if 'telegram_stop_loss' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN telegram_stop_loss BOOLEAN DEFAULT 1")

        if 'email_take_profit' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN email_take_profit BOOLEAN DEFAULT 1")

        if 'email_stop_loss' not in columns:
            migrations.append("ALTER TABLE notification_settings ADD COLUMN email_stop_loss BOOLEAN DEFAULT 1")
        ...
        for migration in migrations:
            try:
                ...
                logger.error(f"Migration failed: {migration}, Error: {e}")
